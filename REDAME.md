# College Website

This repository contains a simple static college website served with GitHub Pages.

How to update:
- Edit `data.js` to change the college name, faculty, branch counts or results.
- Edit `index.html` or `styles.css` to change layout and styling.
- Commit and push changes to `main` (or the branch configured for Pages).

Published at: 'https://jattesanket.github.io/jsweb.github.io/'
